...
""")
    ...
    st.divider()
    st.markdown("## 📸 스냅샷 (내부보고 백업)")
    st.markdown("""
**스냅샷이란?**
내부보고 생성이 완료될 때마다 그 시점의 내부보고 내용을 자동으로 JSON 파일로 저장합니다.
구글시트 탭에 쌓이지 않으므로 메인 스프레드시트가 비대해지지 않습니다.

**저장 위치**
- 로컬 PC: `IBK 게임/snapshots/YYYYMMDD.json`
- Streamlit Cloud(배포판): `git push` 후 반영됨

**스냅샷 확인 방법**
1. 대시보드 좌측 메뉴에서 **📸 스냅샷** 탭 클릭
2. 날짜 선택 드롭다운에서 원하는 날짜 선택
3. 해당 시점의 내부보고 내용을 바로 미리보기 가능

**내부보고 복원 방법**
1. 스냅샷 탭에서 복원할 날짜 선택
2. **⚠️ 이 스냅샷을 내부보고로 복원** 펼치기
3. **복원 실행** 버튼 클릭
4. 구글시트 **내부보고** 탭에 덮어써짐

> 💡 보고 생성 후 `git add IBK 게임/snapshots/ && git push` 하면 배포판 대시보드에서도 스냅샷을 조회·복원할 수 있습니다.
""")
